...
function ilCloudFileList(url_get_block, url_create_folder, url_upload_file, url_delete_item, root_id, root_path, current_id, current_path, max_file_size_txt) {
    ...
    this.encodeBase64Path = function (path) {
        return btoa(encodeURIComponent(path)).replace(/=/g,"Abase64Z");
    }
    
    this.decodeBase64Path = function (path) {
        return decodeURIComponent(atob(path.replace(/Abase64Z/g,"=")));
    }

    var current_path = this.decodeBase64Path(current_path);
    ...
    this.showCurrent = function (show_message, callback, data) {
        ...
        try {
            perm_link = $(".il-footer-permanent-url > a").attr('href');
            ...
            // new
            base64_path = this.encodeBase64Path(current_path);
	        perm_link = perm_link.replace(/path_.*_endPath/,"path_BASE64"+ base64_path+"_endPath");

            $(".il-footer-permanent-url > a").attr('href', perm_link);
            this.showDebugMessage("permalink: " + perm_link);
        } catch (e) {
            this.showDebugMessage(e);
        }
        ...
        //If not load it with ajax
            ...
            self.showDebugMessage("url_get_block: " + url_get_block.replace(/&amp;/g, '&'));
            self.showDebugMessage("current_path: " + current_path);
            self.showDebugMessage("data: " + JSON.stringify({'id': current_id, 'path':current_path}));
            $.ajax({
                type: "POST",
                ...
                data: {'id': current_id, 'path':current_path}
                    ...
                    if (return_data.success) {
                        ...
                        //self.showDebugMessage(return_data.content);
    ...
    this.deleteItem = function (id) {
        self.showDebugMessage("deleteItem");
            ...
            $.ajax({
                ...
                data: {'id': id}
                    ...
                    if (return_data.success) {
                        ...
                        self.hideItemCreationList();
                        $("#xcld_blocks").append(return_data.content);
                        // this is required because the ConfirmationTableGUI does not provide an "ACTIONTARGET" parameter anymore
                        $("#frm_cld_delete_item").attr("target", "cld_blank_target");
                        self.showDebugMessage($("#frm_cld_delete_item").attr("target"));
    ...
    $.address.change(function (event) {
        self.showDebugMessage("address.change: " + event.pathNames);
        if (event.pathNames[0] === "delete_item") {
            self.showDebugMessage("address.change: delete_item: " + event.parameters);
