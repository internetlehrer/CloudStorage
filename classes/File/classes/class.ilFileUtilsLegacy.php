...
class ilFileUtilsLegacy
{
    public static function processZipFile(
        ...
        $filearray = [];
        ilFileUtilsLegacy::recursive_dirscan($a_directory, $filearray);
        ...
        if (!$structure) {
            foreach (array_count_values($filearray["file"]) as $key => $value) {
                ...
                if ($value != "1") {
                    $doublettes .= " '" . ilFileUtilsLegacy::utf8_encode($key) . "'";
                }
            }
    ...
    public static function recursive_dirscan(string $dir, array &$arr): void
    {
        ...
        while (false !== ($file = readdir($dirlist))) {
            ...
            if ($file != '.' && $file != '..') {
                ...
                if (is_dir($newpath)) {
                    ilFileUtilsLegacy::recursive_dirscan($newpath, $arr);
                } else {
                    $arr["path"][] = $dir . "/";
                    $arr["file"][] = end($level);
                }
        ...
        if ($DIR = opendir($directory)) {
            while (($dirfile = readdir($DIR)) !== false) {
                ...
                if (is_file($directory . DIRECTORY_SEPARATOR . $dirfile)) {
                    ...
                    $dirSize = ilFileUtilsLegacy::dirsize(
                        $directory . DIRECTORY_SEPARATOR . $dirfile
                    );
                    if ($dirSize >= 0) {
                        $size += (int) $dirSize;
    ...
    public static function moveUploadedFile(
        ...
        $target_filename = ilFileUtilsLegacy::getValidFilename($target_filename);
    ...
    public static function delDir(string $a_dir, bool $a_clean_only = false): void
    {
        ...
        foreach ($files as $file) {
            if (is_dir(
                ...
                ilFileUtilsLegacy::delDir($a_dir . "/" . $file);
    ...
    {
        ilFileUtilsLegacy::makeDir($a_dir);
    }

    public static function getFileSizeInfo(): string
    {
        global $DIC;
        $size = new DataSize((int) self::getUploadSizeLimitBytes(), DataSize::MB);
    ...
    {
        if ($a_temp_path === null) {
            $temp_path = ilFileUtilsLegacy::getDataDir() . "/temp";
        } else {
            $temp_path = $a_temp_path;
        }

        if (!is_dir($temp_path)) {
            ilFileUtilsLegacy::createDirectory($temp_path);
        }
    ...
    public static function unzip(
        ...
        foreach (new RecursiveIteratorIterator(new RecursiveDirectoryIterator($unzippable_zip_directory)) as $name => $f) {
            ...
            if (is_file($name) && $name !== $sanitizer->sanitize($name)) {
                ...
                $new_name = ilFileUtilsLegacy::getValidFilename($name);
